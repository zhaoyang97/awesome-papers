---
title: "Qwen2.5 / Qwen3 - 阿里通义千问如何把开放模型做成全栈模型族"
year: 2025
...
Qwen Team 在 2025 年 arXiv 技术报告《Qwen3 Technical Report》中做的不是单点 SOTA 论文，而是把 Qwen2.5 到 Qwen3 的开放模型路线压成一套可复用系统：底层仍是 decoder-only Transformer，用自回归目标 $\mathcal{L}_{\text{LM}}=-\sum_t\log p_\theta(x_t\mid x_{<t})$ 吃下 Qwen2.5 的 18T-token 经验，再扩到 Qwen3 约 36T tokens；后训练则把 long-CoT cold start、rule-based reasoning RL、thinking-mode fusion 和 general RL 串起来，让同一个模型能在 `enable_thinking=True/False` 之间切换。它替代的失败 baseline 不是某个模型，而是“开放模型只能追单项榜单”的做法：Qwen3-30B-A3B 只激活 3B 参数却对标更大推理模型，Qwen3-4B 被官方描述为可追平 Qwen2.5-72B-Instruct，而 235B-A22B 把 MoE、推理预算和多语言能力放进同一发布包。更关键的是，后来的 Qwen3-2507、Qwen3-Coder、GSPO、Qwen3-Next、Qwen3-VL、Qwen3-Omni、Qwen3Guard 证明原始 hybrid Qwen3 其实是一个可分叉母体：它后来分别沿模式拆分、agentic coding、sequence-level RL、hybrid attention、高稀疏 MoE、多模态和流式安全长成独立支线。历史影响上，它接上 [Llama 3（2024）](../2024_llama3/) 的开放权重工程化路线，也回应 [DeepSeek-R1（2025）](../2025_deepseek_r1/) 的推理 RL 震动；隐藏 lesson 是：开放模型的壁垒正在从“有没有权重”迁移到“有没有完整生态、数据飞轮和可服务的思考接口，以及能继续分化出专门子家族的系统设计”。
...
### 关键设计 6：GSPO 把原始 reasoning RL 升级成可扩展的 sequence-level RL

原始 Qwen3 已经把 rule-based reasoning RL 放进四阶段后训练，但到 2025 年 7 月，Qwen 团队又专门发布了 GSPO，等于把“为什么后续 Qwen3 分支能继续放大 RL”解释清楚。GSPO 的核心不是再加一个奖励项，而是把重要性比率、裁剪和优化单位从 token 改成 sequence。官方给出的目标函数是：
...
这条式子的重要点有两个。第一，$s_i(\theta)$ 是基于整条 response likelihood 的长度归一化 importance ratio，而不是逐 token 的 noisy ratio；第二，clipping 也在 sequence level 上进行，所以 RL 信号更稳定，对大 MoE 的专家激活抖动更不敏感。Qwen 官方博客明确把这件事和最新的 Qwen3 Instruct、Thinking、Coder 绑在一起，说 GSPO 能持续吃更多训练算力，并且不再依赖 GRPO 在 MoE 场景下常用的 Routing Replay 补丁。

这解释了一个容易被忽略的问题：为什么原始 Qwen3 是“同一个 checkpoint 里能想也能不想”，而后续却开始往 2507 和 Coder 这种更专门的 post-training 分支走？因为一旦 sequence-level RL 足够稳定，团队就可以分别优化 instruction following、代码代理、多轮工具使用和更长 reasoning，而不必把所有目标挤在一个折中 checkpoint 里。GSPO 不是偏离 Qwen3 主线，而是在给 Qwen3 的后续分叉提供训练层的可扩展性。

### 关键设计 7：后续分支不是跑题，而是在拆原始 Qwen3 的隐含耦合

如果把 2025 年 4 月的原始 Qwen3 看成一个 umbrella release，后面的 Qwen3-2507、Qwen3-Coder、Qwen3-Next、Qwen3-VL、Qwen3-Omni、Qwen3Guard 就像是在把最初被绑在一起的几组目标逐步拆开：thinking 与 non-thinking、通用聊天与 agentic coding、标准 Transformer 注意力与超长上下文效率、文本能力与多模态交互、模型内部对齐与运行时安全控制。

| 后续分支 | 专门化了什么 | 官方信号 | 说明原始 Qwen3 哪里有耦合 |
|---|---|---|---|
| Qwen3-2507 | Instruct-only / Thinking-only + 256K 长上下文 | 官方 README 明说是 previous non-thinking mode 和 thinking model 的延续 | 一个 hybrid checkpoint 很优雅，但产品心智和 serving 路由更喜欢拆开 |
| Qwen3-Coder | agentic coding + execution-driven RL | 480B-A35B、7.5T tokens、256K native、20,000 并行环境 | 通用 Qwen3 会写代码，但代理式软件工程需要独立 reward surface |
| Qwen3-Next | hybrid attention + 高稀疏 MoE + MTP | 80B total / 3B active，Gated DeltaNet + Gated Attention，256K native | 原始 Transformer + 常规 MoE 在线性长上下文吞吐上不够激进 |
| Qwen3-VL | 视觉代理、OCR、长视频与空间感知 | 235B-A22B，256K native，可扩到 1M | 文本主干可以迁移到视觉，但视觉时空建模需要独立架构增强 |
| Qwen3-Omni | 音频/视频/语音输出一体化 | 30B-A3B Thinker-Talker，文本/图像/音频/视频输入 | 多模态交互不只是再加一个 encoder，而是要处理实时语音和 turn-taking |
| Qwen3Guard | prompt 级与 token-stream 级安全 | Gen/Stream 双变体，0.6B/4B/8B，token-stream moderation | 安全不再只是后训练残差，而是独立的运行时控制回路 |

从这个角度看，原始 Qwen3 的真正贡献不只是“第一次把 thinking 打开/关闭”，而是定义了一个足够宽的母体，使后续分支都能被解释为对同一底座的专业化改写。2507 拆模式，Coder 拆任务反馈，Next 拆架构效率，VL/Omni 拆输入模态，Guard 拆安全执行。它们不是偏离主叙事，而是在证明最初那个 hybrid Qwen3 的设计空间到底有多大。
...
从 2025 年下半年再回头看，这条“今生”分化得非常清楚：后续 Qwen3 分支几乎是在逐项拆解原始 hybrid release 里的张力。Qwen3-2507 说明 thinking 与 non-thinking 更适合拆成独立产品线；Qwen3-Coder 和 GSPO 说明代码代理与大规模 RL 需要单独的训练目标和基础设施；Qwen3-Next 说明超长上下文效率会反向推动注意力和稀疏结构重写；Qwen3-VL、Qwen3-Omni 和 Qwen3Guard 则说明视觉、语音和安全不会永远只是“LLM 的附属功能”，而会各自长成支线家族。

| 后继分支 | 它拆出了什么 | 官方依据 | 思想史含义 |
|---|---|---|---|
| Qwen3-2507 | hybrid 中的 instruct/thinking 二元性 | GitHub README 明说 previous non-thinking mode 与 thinking model 分线延续 | 推理预算开始从开关变成产品 SKU |
| Qwen3-Coder | 代码与 agent 环境反馈 | 官方 blog 给出 480B-A35B、7.5T tokens、20,000 并行环境 | 通用开放模型开始分化出真正的软件工程支线 |
| GSPO | MoE 上可扩展的 sequence-level RL | 官方 blog 强调 sequence-level ratio/clipping 与去掉 Routing Replay | Qwen3 的 RL 主线从“会用 RL”升级成“RL 可以持续扩张” |
| Qwen3-Next | 超长上下文与高稀疏效率 | 官方 model card 写明 Gated DeltaNet + Gated Attention、80B/3B、256K native | 架构创新重新回到开放模型主线，而不只是部署技巧 |
| Qwen3-VL | 视觉代理、OCR、长视频 | 官方 model card 写明 235B-A22B、256K native、visual agent | Qwen3 文本母体被扩展成可执行视觉系统 |
| Qwen3-Omni | 音频、视频、语音流式输出 | 官方 model card 写明 Thinker-Talker 和 real-time streaming speech | “对话模型”开始接近实时多模态助理 |
| Qwen3Guard | 运行时安全分支 | 官方 blog 写明 Gen/Stream 双变体和 token-stream moderation | 安全从训练附属项变成单独系统层 |
...
这些误读本身也说明 Qwen3 的位置有点复杂。它不是纯研究论文，也不是普通产品公告，而是介于论文、模型仓库、文档、生态工程和品牌叙事之间的综合体。后续的 2507、Coder、Next、VL、Omni、Guard 进一步证明，Qwen3 更像一个母发布，而不是一个封闭终点。对 awesome-papers 来说，值得记录的正是这种“论文形态变化”：经典 AI 论文越来越像系统发布。

### 一张图：从开放权重到可控思考

Qwen3 的思想史可以压缩成一句话：**开放模型从“我能下载权重”走向“我能控制模型怎么想、怎么用工具、怎么部署，并且能把这些能力继续拆成专门分支”。** LLaMA 打开权重，Llama 3 把开放权重做成前沿工程报告，DeepSeek-R1 把推理 RL 拉到开源社区，Qwen3 则把这些能力收束进一个可选择尺寸、可选择 dense/MoE、可选择 thinking/non-thinking、可选择部署框架的模型族，随后又分化出 Coder、Next、VL、Omni 和 Guard 这样的系统级支线。
...
第四个站得住的判断，是原始 Qwen3 其实定义了一个“可分叉的母体”，而不是一个最终形态。后面的 Qwen3-2507、Qwen3-Coder、GSPO、Qwen3-Next、Qwen3-VL、Qwen3-Omni、Qwen3Guard 看起来分散，实际上都在验证同一件事：开放模型真正的竞争单位不再是单个 checkpoint，而是一个能沿不同产品压力线持续分化的系统家族。
...
| 模型族会继续分叉成子家族 | 4 月是 hybrid 母发布 | 2507/Coder/Next/VL/Omni/Guard 各自成线 | 分叉后还能否保持统一接口 |
...
如果把这些修正放在一起，会更清楚地看到后续分支的意义：Qwen3-2507 修正的是模式耦合，Qwen3-Coder 修正的是“代码只是通用能力子集”的假设，GSPO 修正的是“现有 RL 算法自然能扩到大 MoE”的假设，Qwen3-Next 修正的是“原始 Transformer 注意力足够承接 256K+ agent 工作流”的假设，Qwen3-VL / Qwen3-Omni 修正的是“文本主干稍加适配就够”的假设，Qwen3Guard 修正的是“安全主要留在离线对齐阶段”的假设。

| 后续分支 | 它修正了什么 | 说明了什么 |
|---|---|---|
| Qwen3-2507 | 一个 hybrid checkpoint 可以优雅服务所有人 | 产品与 serving 往往更偏好清晰分线 |
| Qwen3-Coder | 通用 post-training 足以覆盖代理式软件工程 | 代码代理需要环境反馈和单独 RL 面 |
| GSPO | GRPO 式 token-level RL 自然能扩到大 MoE | sequence-level RL 才更稳、更省基础设施 |
| Qwen3-Next | 原始注意力与 MoE 形态足够高效 | 超长上下文会逼出新的 attention/sparsity 设计 |
| Qwen3-VL / Qwen3-Omni | 文本骨干稍作适配即可吃下多模态 | 真正多模态需要专门的视觉时空与语音架构 |
| Qwen3Guard | 安全主要是训练后过滤问题 | 安全也需要运行时 token-stream 控制 |
...
更重要的是，这条主线后来没有停在“再发几个更强 checkpoint”，而是沿着六条很具体的方向裂变：2507 把模式拆开，Coder 把软件工程抬到主线，GSPO 把 RL 稳定性补成基础设施，Next 把架构效率推回前台，VL/Omni 把多模态从附件变成主体，Guard 则把安全外置成实时系统组件。回头看，这些后续分支不是在稀释 Qwen3，而是在把原始 Qwen3 的设计意图讲得更明确。
...
值得期待的方向包括：更公开的小规模 RL 复现实验，更可靠的 tool-use reward，更强的多语言安全评测，视觉/音频/文本统一模型，更低成本的 MoE serving，以及能根据任务自动分配 thinking budget 的 scheduler。Qwen3 已经证明“模型族”是一个有效组织方式；而 2507、Coder、GSPO、Next、VL、Omni、Guard 这些分支又进一步说明，未来开放社区真正要验证的，是这些子家族能否继续共享接口、共享数据飞轮和共享 agent 基础设施。
