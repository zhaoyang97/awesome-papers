...
> **2025 年 8 月 7 日，OpenAI 发布 [Introducing GPT-5](https://openai.com/index/introducing-gpt-5/) 与 [GPT-5 System Card](https://openai.com/index/gpt-5-system-card/)，第一次把“快模型、深思模型、mini 变体与实时 router”打包成一个统一入口。** 这篇记录真正新鲜的地方，不是又多了一个神秘大模型名字，而是 OpenAI 公开承认：今天最难的问题已经不是单个模型能否答对，而是系统能否判断什么时候该快、什么时候该想久一点、什么时候该调用工具、什么时候该诚实停下。后续的 [GPT-5.2 update](https://openai.com/index/gpt-5-system-card-update-gpt-5-2/) 和 [GPT-5.5 System Card](https://openai.com/index/gpt-5-5-system-card/) 又把 monitorability、destructive actions、computer use 与跨工具工作继续纳入同一家族叙事。于是 GPT-5 更像一个分水岭：从这一代起，前沿模型的核心竞争点开始从“谁分数更高”转向“谁更像一个会调度自己、会承认边界、会持续完成工作的系统”。
...
GPT-5 取代的失败 baseline，也不是某个单一学术模型，而是让用户先在 [GPT-4o](2024_gpt4o.md) 与 [OpenAI o1](2024_o1.md) 一类入口之间自己猜任务难度的旧交互。官方数字说明这种系统化调度确实有现实价值：AIME 2025 达到 94.6%，固定 477 题的 SWE-bench Verified 为 74.9%，而在更能暴露诚实边界的失败环境里，gpt-5-thinking 把缺图 CharXiv 的 deception rate 从 o3 的 0.87 降到 0.09，把 broken-tools browsing 从 0.61 降到 0.11，把 impossible coding 从 0.47 降到 0.17。真正反直觉的 lesson 是，GPT-5/5.x 留下来的历史影响并不首先是某个已公开的新层或新参数规模，而是把“模型选择、测试时计算、安全监控与工具状态”一起写进系统卡对象；这条线随后直接延伸到 [GPT-5.5 System Card](https://openai.com/index/gpt-5-5-system-card/) 所强调的 documents、computer use 与跨工具完成工作。
...
### 家族更新：5.2 与 5.5 把重点从“会路由”推到“能持续做事”
...
- **截至 2026-09-14，历史结论仍必须暂定。** 家族更新只有约一年，架构与训练细节未公开，外部长期复现、事故率、路由公平性和真实生产分布都不足以做“最终胜者”判断。
...
- **跨学科外溢**：截至 2026-09-14，最可核的外溢发生在安全治理、HCI、软件工程评测和生物/网络风险测量，而非自然科学中的架构迁移。HealthBench、biological troubleshooting、protein binding、cyber range 与真实知识工作被纳入同一系统卡，但这只能证明评测与部署问题跨学科，不能证明 GPT-5 已在这些领域取代专家。

这一“今生”至少包含十项直接官方工件：safe-completions、GPT-5 developer release、5.1 addendum、5.2 release、5.2 card、5.3-Codex card、5.4 release、5.4 card、monitorability release、5.5 release、5.5 card 及其两次更新。它们共同说明 GPT-5 更像一个持续更新的系统家族，而不是一篇发表后冻结的单篇论文。
...
- **“2026 年已经能给 GPT-5 定历史胜负。”** 截至 9 月 14 日，主家族只有约一年公开历史，命名、评测和 safeguards 持续更新，5.5 卡甚至在 8 月修正关键 pass@4 数字。长期事故、用户依赖、路由偏差、外部复现和真实成本仍不足，当前只能判断范式影响，不能判断最终稳定性。
...
- **披露不够支持复现。** GPT-5、5.2 与 5.5 都只公开了数据来源类别、reasoning model 经过 reinforcement learning、router 使用哪些高层信号，以及若干 benchmark 与 safeguards；但参数量、训练 token、架构细节、奖励构造、路由阈值、校准过程和大多数 serving 设计都未公开。对于一篇思想史意义很强的系统卡，这种不透明会让外界很容易把产品叙事误读为方法论文。
