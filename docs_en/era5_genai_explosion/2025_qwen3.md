---
title: "Qwen2.5 / Qwen3 - How Alibaba Turned Open LLMs into a Full-Stack Model Family"
year: 2025
...
### Key Design 6: GSPO Upgrades the Original Reasoning RL into Scalable Sequence-Level RL

The original Qwen3 release already placed rule-based reasoning RL inside its four-stage post-training recipe, but the July 2025 GSPO release effectively explains why later Qwen3 branches could keep scaling RL. The key idea is not just another reward term. GSPO changes the importance ratio, clipping, and optimization unit from the token level to the sequence level. The official objective is:

$$
J_{\text{GSPO}}(\theta)=\mathbb{E}\left[\frac{1}{G}\sum_{i=1}^{G}\min\left(s_i(\theta)A^i,\operatorname{clip}(s_i(\theta),1-\epsilon,1+\epsilon)A^i\right)\right],\quad
s_i(\theta)=\left(\frac{\pi_\theta(y_i\mid x)}{\pi_{\theta_{\text{old}}}(y_i\mid x)}\right)^{1/|y_i|}.
$$

Two points matter here. First, $s_i(\theta)$ is a length-normalized importance ratio based on full-sequence likelihood rather than noisy token-wise ratios. Second, clipping also happens at the sequence level, so the RL signal is more stable and less sensitive to expert-activation volatility in large MoE models. The official GSPO blog ties this directly to the latest Qwen3 Instruct, Thinking, and Coder models, arguing that GSPO keeps improving with more training compute and removes the dependence on GRPO's Routing Replay workaround in MoE RL.

This clarifies an easy-to-miss question: why was the original Qwen3 framed as one checkpoint that can think or not think, while later releases move toward more specialized post-training branches such as 2507 and Coder? Because once sequence-level RL is stable enough, the team can optimize instruction following, coding agents, multi-turn tool use, and longer reasoning separately instead of squeezing every objective into one compromise checkpoint. GSPO is not a detour away from Qwen3. It is the training-layer explanation for why the Qwen3 line could branch without losing coherence.

### Key Design 7: The Later Branches Are Not Side Quests but a Decomposition of the Original Couplings

If the April 2025 Qwen3 release is treated as an umbrella release, then Qwen3-2507, Qwen3-Coder, Qwen3-Next, Qwen3-VL, Qwen3-Omni, and Qwen3Guard look like a gradual decomposition of goals that were initially bundled together: thinking and non-thinking, general chat and agentic coding, standard Transformer attention and ultra-long-context efficiency, text capability and multimodal interaction, internal alignment and runtime safety control.

| Later branch | What it specializes | Official signal | What it reveals about original Qwen3 |
|---|---|---|---|
| Qwen3-2507 | Instruct-only / Thinking-only plus 256K long context | The official README explicitly presents it as the continuation of the previous non-thinking mode and thinking model | One hybrid checkpoint is elegant, but product mental models and serving routes often prefer separation |
| Qwen3-Coder | Agentic coding plus execution-driven RL | 480B-A35B, 7.5T tokens, 256K native context, 20,000 parallel environments | General Qwen3 can code, but agentic software engineering needs its own reward surface |
| Qwen3-Next | Hybrid attention, high-sparsity MoE, and MTP | 80B total / 3B active, Gated DeltaNet plus Gated Attention, 256K native context | The original Transformer plus moderate MoE path was not aggressive enough on ultra-long-context throughput |
| Qwen3-VL | Visual agents, OCR, long video, and spatial perception | 235B-A22B, 256K native context, expandable to 1M | A text backbone transfers to vision, but visual-temporal modeling still needs its own architectural upgrades |
| Qwen3-Omni | Unified audio, video, and speech output | 30B-A3B Thinker-Talker with text, image, audio, and video inputs | Multimodal interaction is not merely adding an encoder; it must handle real-time speech and turn-taking |
| Qwen3Guard | Prompt-level and token-stream safety | Gen and Stream variants at 0.6B/4B/8B with token-stream moderation | Safety is no longer only a post-training residue; it becomes a runtime control loop |

From this angle, the deepest contribution of the original Qwen3 release is not merely that it first exposed a thinking switch. It defined a broad enough parent design that the later branches all read like specializations of the same substrate. 2507 separates modes, Coder separates task feedback, Next separates architectural efficiency, VL and Omni separate input modalities, and Guard separates safety execution. They do not break the main narrative. They reveal how large the original hybrid Qwen3 design space actually was.
...
Seen again from the second half of 2025, that “after” branch becomes very clear: later Qwen3 releases almost itemize the tensions hidden inside the original hybrid release. Qwen3-2507 shows that thinking and non-thinking are often better served as separate product lines. Qwen3-Coder and GSPO show that coding agents and large-scale RL need their own objectives and infrastructure. Qwen3-Next shows that ultra-long-context efficiency pushes attention design and sparsity back into the architectural foreground. Qwen3-VL, Qwen3-Omni, and Qwen3Guard show that vision, speech, and safety will not remain mere accessories to an LLM; they become branch families of their own.

| Descendant branch | What it separates out | Official cue | Intellectual meaning |
|---|---|---|---|
| Qwen3-2507 | The instruct/thinking duality inside hybrid Qwen3 | The GitHub README explicitly frames it as the continuation of the previous non-thinking mode and thinking model | Reasoning budget starts turning from a toggle into product SKUs |
| Qwen3-Coder | Coding and agent-environment feedback | The official blog gives 480B-A35B, 7.5T tokens, and 20,000 parallel environments | General open models begin to split into genuine software-engineering branches |
| GSPO | Scalable sequence-level RL on MoE models | The official blog emphasizes sequence-level ratio/clipping and removal of Routing Replay | Qwen3's RL story upgrades from “uses RL” to “can keep scaling RL” |
| Qwen3-Next | Ultra-long context and high-sparsity efficiency | The official model card specifies Gated DeltaNet plus Gated Attention, 80B/3B, and 256K native context | Architectural innovation returns to the open-model mainline rather than staying only a serving trick |
| Qwen3-VL | Visual agents, OCR, and long video | The official model card specifies 235B-A22B, 256K native context, and visual-agent ability | The Qwen3 text parent expands into an executable visual system |
| Qwen3-Omni | Audio, video, and streaming speech output | The official model card specifies Thinker-Talker and real-time streaming speech | “Chat model” starts approaching a real-time multimodal assistant |
| Qwen3Guard | Runtime safety branching | The official blog specifies Gen and Stream variants with token-stream moderation | Safety becomes a system layer rather than only a training aftereffect |
...
graph TD
    ...
    GSPO[GSPO 2025<br/>sequence-level RL scaling]
    Qwen2507[Qwen3-2507<br/>separate instruct/thinking]
    QwenCoder[Qwen3-Coder<br/>agentic coding]
    QwenNext[Qwen3-Next<br/>hybrid attention + high-sparsity MoE]
    QwenVL[Qwen3-VL<br/>visual agent branch]
    QwenOmni[Qwen3-Omni<br/>omni-modal thinker-talker]
    QwenGuard[Qwen3Guard<br/>streaming safety]
...
These misreadings also show why Qwen3's position is complex. It is not a pure research paper, nor an ordinary product announcement. It sits between paper, model repository, documentation, ecosystem engineering, and brand narrative. The later 2507, Coder, Next, VL, Omni, and Guard branches further show that Qwen3 is better understood as a parent release than a closed endpoint. For awesome-papers, that is exactly why it is worth recording: classic AI papers are increasingly becoming system releases.

### One Diagram: From Open Weights to Controllable Thinking

Qwen3's lineage can be compressed into one sentence: **open models move from “I can download the weights” to “I can control how the model thinks, uses tools, gets deployed, and later specialize each of those abilities into distinct branches.”** LLaMA opened the weights; Llama 3 turned open weights into frontier engineering; DeepSeek-R1 brought reasoning RL into the open community; Qwen3 gathers those capabilities into a model family where users can choose size, dense or MoE, thinking or non-thinking, and deployment framework, and later watch those same axes split into Coder, Next, VL, Omni, and Guard.
...
The fourth durable judgment is that the original Qwen3 release defined a branchable parent design rather than a final form. The later Qwen3-2507, Qwen3-Coder, GSPO, Qwen3-Next, Qwen3-VL, Qwen3-Omni, and Qwen3Guard releases look scattered only on the surface. In practice, they all validate the same claim: the real competitive unit for open models is no longer a single checkpoint but a system family that can keep specializing along different product-pressure lines.
...
| Model families keep splitting into subfamilies | April 2025 is a hybrid parent release | 2507/Coder/Next/VL/Omni/Guard all become separate lines | Whether branching can preserve a unified interface |
...
If those revisions are read together, the later branches become easier to interpret. Qwen3-2507 revises the mode-coupling assumption. Qwen3-Coder revises the idea that coding is just a subset of general post-training. GSPO revises the assumption that existing RL algorithms naturally scale to large MoE training. Qwen3-Next revises the belief that the original Transformer attention path is enough for 256K-plus agent workloads. Qwen3-VL and Qwen3-Omni revise the belief that a text backbone only needs light adaptation to absorb multimodality. Qwen3Guard revises the assumption that safety mostly belongs to offline alignment.

| Later branch | What it revises | What that means |
|---|---|---|
| Qwen3-2507 | One hybrid checkpoint can elegantly serve everyone | Product and serving systems often prefer clearer mode separation |
| Qwen3-Coder | General post-training is enough for agentic software engineering | Coding agents need environment feedback and their own RL surface |
| GSPO | GRPO-style token-level RL naturally scales to large MoE models | Sequence-level RL is more stable and infrastructure-friendly |
| Qwen3-Next | The original attention and MoE form is efficient enough | Ultra-long context pushes new attention and sparsity designs |
| Qwen3-VL / Qwen3-Omni | A text backbone with light adaptation is enough for multimodality | Real multimodality needs dedicated visual-temporal and speech architectures |
| Qwen3Guard | Safety is mainly a post-training filtering problem | Safety also needs runtime token-stream control |
...
More importantly, this mainline did not stop at “release a few stronger checkpoints.” It split along six concrete directions: 2507 separates modes, Coder moves software engineering into the main story, GSPO turns RL stability into infrastructure, Next brings architectural efficiency back to center stage, VL and Omni make multimodality a first-class branch, and Guard externalizes safety into a real-time systems component. In hindsight, those later branches do not dilute Qwen3. They make the intent of the original Qwen3 design much clearer.
...
Promising directions include more public small-scale RL reproductions, more reliable tool-use reward, stronger multilingual safety evaluation, unified vision/audio/text models, lower-cost MoE serving, and schedulers that allocate thinking budget automatically by task. Qwen3 has already shown that “model family” is an effective organizing unit; and the 2507, Coder, GSPO, Next, VL, Omni, and Guard branches further suggest that the next open question is whether these subfamilies can keep sharing interfaces, data flywheels, and agent infrastructure instead of becoming isolated product silos.
...
- GSPO official blog: <https://qwenlm.github.io/blog/gspo/>
- Qwen3Guard official blog: <https://qwenlm.github.io/blog/qwen3guard/>
